...
proposal <- charger_rdata(paste0(data_source,"datas.RData"), "proposal")
project <- charger_rdata(paste0(data_source,"datas.RData"), "project")
...
proposal <- proposal %>% dplyr::rename(status = STATUT_EVAL) %>% 
  ...
  select(-ELIGIBLE, -programme, -area, -panel_2, -panel_1, -panel1_tri, -panel2_tri, -programme_tri, -area_tri, -pilier_tri,
...
project <- project %>% dplyr::rename(status = STADE_PROJET) %>%
  ...
  select(-programme, -area, -panel_2, -panel_1, -panel1_tri, -panel2_tri, -programme_tri, -area_tri, -pilier_tri,
         -action_1, -action_2, -action_3, -action2_tri, -action1_tri, -action3_tri, -pilier_abbr)
...
project <- project %>% 
  mutate(url=paste0("https://cordis.europa.eu/project/id/", NUM_PROJET))
...
# url1="http://ec.europa.eu/research/participants/portal/data/call/h2020/calls.json"
# appels_h2020 <- fromJSON(file=url1) %>% 
#   .[["callData"]] %>%
#   .[["Calls"]]
# app <- appels_h2020$CallIdentifier$CallId[0]
# app <- tibble::data_frame(APPEL = appels_h2020$CallIdentifier$CallId,call_name = appels_h2020$Title)
# 
# project <- 
#   tibble::data_frame(APPEL = appels_h2020$CallIdentifier$CallId,call_name = appels_h2020$Title) %>% 
