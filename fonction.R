...
annee_call <- function(tab1) {
  year = str_extract_all(tab1$APPEL, "[^H2020]([[:digit:]]{4})")
  year = str_extract_all(year, "[0-9]{4}")
  map(year, function(x){ifelse(length(x)== 1, x, "null")})
}
...
detect_pays_zz <- function(tab1) {
  ...
  tab_zz <- 
    ...
    select(CODE_ORG, NOM_LONG, NOM_COURT) %>% 
    unique()
...
#dans la boucle, le prog va créer un niveau à la fois et lui attribuer les libellés correspondants
  for (i in mln){
   tab1 <- tab1 %>%  mutate( tt = decoupe_variable(CODE_THEME, i)) %>% 
      left_join(themes[,c("CODE_THEME", "abbr", "theme")], by = c("tt" = "CODE_THEME"), all.x =TRUE)
